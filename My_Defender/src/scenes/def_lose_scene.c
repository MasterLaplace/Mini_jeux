...
extern const size_t NB_LOSE_BUTTONS;
extern const preset_button_t LOSE_BUTTONS[];
extern const size_t NB_LOSE_INTERFACE;
extern const preset_interface_t LOSE_INTERFACE[];
...
static link_t *define_game_buttons(game_t *game)
{
    ...
    for (size_t i = 0; i < NB_LOSE_BUTTONS; i++) {
        button = create_button(LOSE_BUTTONS[i], game->textures);
...
static link_t *define_game_interface(game_t *game)
{
    ...
    for (size_t i = 0; i < NB_LOSE_INTERFACE; i++) {
        element = create_interface(LOSE_INTERFACE[i], game->textures);
