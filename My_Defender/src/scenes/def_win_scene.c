...
extern const size_t NB_WIN_BUTTONS;
extern const preset_button_t WIN_BUTTONS[];
extern const size_t NB_WIN_INTERFACE;
extern const preset_interface_t WIN_INTERFACE[];
...
static link_t *define_game_buttons(game_t *game)
{
    ...
    for (size_t i = 0; i < NB_WIN_BUTTONS; i++) {
        button = create_button(WIN_BUTTONS[i], game->textures);
...
static link_t *define_game_interface(game_t *game)
{
    ...
    for (size_t i = 0; i < NB_WIN_INTERFACE; i++) {
        element = create_interface(WIN_INTERFACE[i], game->textures);
