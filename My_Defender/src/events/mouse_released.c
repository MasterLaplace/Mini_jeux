...
static button_t *config_button(mouse_action_t *action, link_t *actual)
{
    ...
    if (((button_t *) actual->obj)->callback == &callback_level_button)
        action->type = 2;
...
static void call_callback(game_t *game, button_t *button, link_t *actual,
                        mouse_action_t *action)
{
    if (action->type == 0)
        button->callback(game, NULL);
    if (action->type == 1)
        button->callback(game, (tower_t *) actual->obj);
    if (action->type == 2)
                        ...
                        sfMouseButtonEvent mouse)
{
    ...
    if (!cursor_in_sprite(game->game_scene->wheel->sprite, mouse.x, mouse.y))
